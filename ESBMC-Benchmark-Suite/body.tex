...
\begin{table}[t]
  \centering\small
  \caption{Cross-tool verdicts. Both tools agree with the ground truth wherever both conclude;
           nuXmv proves two edge-detector benchmarks that ESBMC-PLC's $k$-induction leaves open.}
  \label{tab:crosstool}
  \begin{tabular}{@{}lccc@{}}
    \toprule
    & \textbf{ESBMC-PLC v8.4} & \textbf{nuXmv 2.2.0} & \textbf{Ground truth}\\
    & (SMT-BMC/$k$-ind., native XML) & (BDD/IC3, translated SMV) & \\
    \midrule
    ...
    stateful: edge detectors (2) & \emph{unknown} ($k$-ind.) & \textbf{proved} & \textsc{safe}\\
    \bottomrule
  \end{tabular}
\end{table}
