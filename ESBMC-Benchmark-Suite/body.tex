...
\begin{itemize}
  ...
  \item \textbf{Cross-tool baseline results} (\S\ref{sec:eval}) on the full suite: all 25
    ...
    truth), and an independent model checker (nuXmv) agrees on all 24 interlock variants, an
    architecturally independent confirmation of the ground truth. The evaluation also quantifies
...
proved and every faulted variant refuted (Table~\ref{tab:crosstool}). Two mature tools with
independent decision procedures reaching identical verdicts is the strongest evidence
available that the ground truth for these benchmarks is tool-neutral, not an artefact of the
tool that helped establish it.

\begin{table}[t]
  \centering\small
  \caption{Cross-tool verdicts on the 12 combinational interlock benchmarks (24 variants).}
  \label{tab:crosstool}
  \begin{tabular}{@{}lccc@{}}
    \toprule
    ...
    \midrule
    safe variants (12)      & 12 proved   & 12 proved   & \textsc{safe}\\
    faulted variants (12)   & 12 refuted  & 12 refuted  & \textsc{violation}\\
    agreement               & \multicolumn{2}{c}{\textbf{24/24 (100\%)}} & ---\\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Scope of these numbers.}
The cross-tool agreement covers the twelve interlock benchmarks that translate to finite-state
SMV invariants. The termination and stateful benchmarks are not expressible as such without
further encoding and remain single-tool; and PLCverif, which consumes Structured Text rather
than PLCopen~XML, likewise requires translation. A complete multi-tool matrix across all three
languages---and the PLCopen~XML normalisation that would let each tool ingest the whole
