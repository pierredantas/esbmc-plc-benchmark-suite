...
Only PLCopen XML programs run: ESBMC picks its LD front end from the file extension
and XML-parses whatever it finds, so the textual .ld DSL and the .st programs are
skipped rather than reported as failures. A termination property is not accepted by
--ld-props at all; those variants go through the scan watchdog instead.
"""
import argparse
import pathlib
import shutil
...
def variants():
    """Every runnable (benchmark, variant) pair, with its property file."""
    for meta_path in sorted((ROOT / "benchmarks").glob("*/*/benchmark.yml")):
        ...
        for var in meta.get("variants", []):
            ...
            if program.suffix == ".xml" and props.exists():
